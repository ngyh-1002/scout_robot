...
* **상황:** 로봇크기(58cm,62cm) + 인플레이션(35cm)을 크게주면 좁은 통로(78cm)에서 Nav2가 경로를 생성하지 못함
* **문제:** 실제상황에 맞는 로봇 크기 + 인플레이션을 주면 경로생성 안됨, 실제보다 작게주면 충돌 발생
* **원인:** 네비게이션 콘솔 출력값에 글로벌 플래너가 경로가 없다고 판단하는게 원인.
           실시간 속도,방향,장애물과의 거리를 기반으로 계산하는 로컬 플래너는 현실반영된 cost값으로 경로계산 가능, 글로벌 플래너는 통로를 지나갈 수 없다 판단
* **해결방안:** 글로벌 플래너의 로봇 크기를 줄이고 인플레이션, 로컬 플래너에서 실제 footprint로 충돌 회피 담당시키기
