...
* 쿼터니언은 3차원 회전을 **수학적으로 안정적으로** 표현하기 위한 방식이에요.
* 오일러각은 회전 순서에 따라 결과가 달라지는 **짐벌락(Gimbal Lock)** 문제가 있지만,
  쿼터니언은 그런 문제가 없습니다.
* ROS2에서는 로봇의 자세(`orientation`)를 쿼터니언으로 표현합니다.

> 즉,
>
> * 오일러각은 “인간이 보기 좋게”,
> * 쿼터니언은 “컴퓨터가 계산하기 좋게” 쓰는 표현이에요.
...
>   **z = sin(θ/2)**
>   **w = cos(θ/2)**
> * `atan2(x,y)`는 2개의 인자를 쓰는사분면 정보를 포함한 정확한 각도 계산 함수이다.
